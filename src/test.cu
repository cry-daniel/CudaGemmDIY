...
int main(){
    ...
    for (int i = 0; i < M_MAT_A_ROW_NUM * K_MAT_A_COLUMN_NUM; i++){
        ...
        // printf("h_a[i]: %.5f, h_a_half[i]: %.5f\n", h_a[i], static_cast<float>(h_a_half[i]));
    }
    ...
    for(int i=0;i<SCALE;i+=max(1, SCALE/10))
        printf("cpu[%d]: %5f, tc[%d]: %5f, gpu[%d]:%5f, wmma_half[%d]:%5f\n", i, h_c_cpu[i], i, h_c_cublas_tc[i], i, h_c_naive_cuda[i], i, static_cast<float>(h_c_wmma_tc_half[i]));
        // printf("cpu[%d]: %5f, tc[%d]: %5f, gpu[%d]:%5f, wmma_half[%d]:%5f, wmma_float[%d]:%5f\n", i, h_c_cpu[i], i, h_c_cublas_tc[i], i, h_c_naive_cuda[i], i, static_cast<float>(h_c_wmma_tc_half[i]), i, h_c_wmma_tc[i]);
#endif
